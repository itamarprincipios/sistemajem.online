...
# Read the file
with open('operator/dashboard.php', 'r', encoding='utf-8') as f:
    content = f.read()

# New renderGroups function
new_function = '''function renderGroups() {
...
# Find and replace the renderGroups function
pattern = r'function renderGroups\(\) \{[\s\S]*?\n\}(?=\s*\n\s*(//|function|async))'
content = re.sub(pattern, new_function, content)

# Write back
with open('operator/dashboard.php', 'w', encoding='utf-8') as f:
    f.write(content)
