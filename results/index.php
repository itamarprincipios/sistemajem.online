...
<body>
    ...
    <script>
        ...
        const CURRENT_TENANT_SLUG = "<?php echo $_GET['tenant'] ?? ''; ?>";
        ...
        async function load() {
            try {
                ...
                const [matchRes, awardRes] = await Promise.all([
                    fetch(`/api/matches-api.php?action=list&event_id=${EVENT_IDS}&tenant=${CURRENT_TENANT_SLUG}&_t=${Date.now()}`),
                    fetch(`/api/awards-api.php?event_id=${EVENT_IDS.split(',')[0]}&tenant=${CURRENT_TENANT_SLUG}&modality_id=${state.modality || 0}&category_id=0&_t=${Date.now()}`) // Basic fetch to warm up
                ]);
        ...
        function render() {
            ...
            const loadAwards = async () => {
                ...
                try {
                    ...
                    const res = await fetch(`/api/awards-api.php?event_id=${catEventId}&tenant=${CURRENT_TENANT_SLUG}&modality_id=${state.modality}&category_id=${catId}&gender=${gender}&_t=${Date.now()}`);
