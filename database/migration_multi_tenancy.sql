...
-- Registration Requests (Esses geralmente vêm de fora, mas vinculamos à secretaria se possível)
...
-- Remover unique temporário para recriar com secretaria_id se necessário
ALTER TABLE registrations DROP INDEX unique_registration;
...
-- Enrollments (Não precisa de secretaria_id diretamente pois já ligam Student e Registration que têm, mas pode ser útil para performance)
-- Decidi não colocar em enrollments para manter normalizado, mas vou colocar em Competition Event e Matches.
...
ALTER TABLE users MODIFY COLUMN secretaria_id INT DEFAULT NULL; -- Pode ser null para super_admin
